...
#Initialization
userlimit="/usr/local/shadowsocksr/timelimit.db"
if [[ ! -e userlimit ]]; then
	touch userlimit
fi
...
List_port_user(){
	...
	cd /usr/local/shadowsocksr
        ...
        do
				...
				datelimit=$(cat ${userlimit} | grep "${user_port}:" | awk -F":" '{ print $2 }' | sed 's/\([0-9]\{4\}\)\([0-9]\{2\}\)\([0-9}\{2\}\)\([0-9]\{2\}\)\([0-9]\{2\}\)/\1年\2月\3日 \4:/')
				if [[ -z ${datelimit} ]];then
					datelimit="永久"
				fi
                user_list_all=${user_list_all}"用户名: ${Green_font_prefix} "${user_username}"${Font_color_suffix}\t 端口: ${Green_font_prefix}"${user_port}"${Font_color_suffix}\t 有效期: ${Green_font_prefix}"${datelimit}"${Font_color_suffix}\t  \n"
	done
...
EasyAdd(){
	...
	if [[ ${lsid} == 1 ]];then
		...
		if [[ -z ${checkuid} ]];then
			...
			bash /usr/local/shadowsocksr/timelimit.sh a ${port} ${limit} || EasyAdd
...
EasyEdit(){
	...
	bash /usr/local/shadowsocksr/timelimit.sh e ${port} ${limit} || EasyEdit
	datelimit=$(cat ${userlimit} | grep "${port}:" | awk -F":" '{ print $2 }' | sed 's/\([0-9]\{4\}\)\([0-9]\{2\}\)\([0-9}\{2\}\)\([0-9]\{2\}\)\([0-9]\{2\}\)/\1年\2月\3日 \4:/')
	if [[ -z ${datelimit} ]];then
		datelimit="永久"
	fi
